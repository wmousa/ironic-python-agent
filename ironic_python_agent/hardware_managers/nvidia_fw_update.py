...
"""
Example of config.yaml configuration file:
[
{
"selectors": {
  "devices": ["1018"],
...
def parse_mstflint_query_output(out):
    ...
    :return: dict of query attributes
    """
    query_info = {}
...
class NvidiaNicFirmwareOps(object):
    ...
    def get_nic_psid(self):
        ...
        :return: string, the psid of the nic device
        """
    ...
    def need_reset_before_config(self, force_update=False):
        """ Check if nic device requires firmware reset before applying any
        configurations on the device.
        :param force_update: if force update, reset the device
...
class NvidiaFirmwareBinariesFetcher(object):
    ...
    def get_firmware_binaries(self):
        ...
        if not parser.matches:
            LOG.warning("No bin Files found in the provided URL: %s",
                        self.url)
        fw_bins = []
        threads = []
...
class NvidiaNicConfig(object):
    ...
    def _mstconfig_parse_data(self, data):
        ...
        :return: dict of nic configuration
        """
        #
...
def filter_by_selector(all_devs, selector_list, selector):
    ...
    filtered_dev = []
    for dev in all_devs:
        if getattr(dev, selector) in selector_list:
            filtered_dev.append(dev)
    return filtered_dev


def filter_cfgs(nvidia_nics, cfgs):
    """ Filter the cfgs list according to system nics and return
    a dict of pcis with params to be processed and a
    dict of pcis with force_update value to be forced updated
    or not.
    :param nvidia_nics:  object of NvidiaNics, (list of NvidiaNic)
    :param cfgs: list of cfgs
    :return: dict of pcis with params to be processed,
             dict of pcis with force_update value to be forced updated
    """
    pcis_to_be_processed = {}
    force_update_dict = {}
    for cfg in cfgs:
        if "selectors" not in cfg:
            continue
        selectors = cfg.get("selectors")
        seen_nics = set()
        filtered_devs = nvidia_nics
        if selectors.get("devices"):
            filtered_devs = filter_by_selector(filtered_devs, cfg["selectors"]["devices"], "dev_id")
        if selectors.get("psidList"):
            filtered_devs = filter_by_selector(filtered_devs, cfg["selectors"]["psidList"], "dev_psid")
        if selectors.get("pciList"):
            filtered_devs = filter_by_selector(filtered_devs, cfg["selectors"]["pciList"], "dev_pci")

        for dev in filtered_devs:
            params = {}
            prefix = dev.dev_pci[:-1]
            is_seen_nic = prefix in seen_nics
            first_device = prefix + "0"
            if not is_seen_nic:
                seen_nics.add(prefix)
                if cfg.get("globalConfig"):
                    params.update(cfg.get("globalConfig"))
            # Add only the first device of the nic to orce_update_dict dict
            force_update_dict[first_device] = \
                force_update_dict.get(first_device) or \
                cfg.get("forceFwUpdate", False)

            is_first_device = dev.dev_pci[-1] == '0'
            if is_first_device and cfg.get("device1Config"):
                params.update(cfg.get("device1Config"))
            elif not is_first_device and cfg.get("device2Config"):
                params.update(cfg.get("device2Config"))

            if params:
                if pcis_to_be_processed.get(dev.dev_pci):
                    pcis_to_be_processed[dev.dev_pci].update(params)
                else:
                    pcis_to_be_processed[dev.dev_pci] = params

    return pcis_to_be_processed, force_update_dict


def fw_update_if_needed(psid_map, force_update_dict):
    """ Update firmware according the psid map and force-update dict
    :param psid_map: dict mapping between PSID and an image_info dict
    :param force_update_dict: dict of pcis with force_update value to
    be forced updated
    :return: None
    """
    for dev, force_update in force_update_dict.items():
        try:
            ...
            # Note: device Reset is required if a firmware version was burnt
            if dev_ops.need_reset_before_config(force_update):
                dev_ops.reset_device()
        except Exception as e:
            LOG.error("Failed to process device %s. %s", dev, str(e))


def set_devices_config(pcis_to_be_processed):
    """ Set configuration according to dict of pcis with params.
    :param pcis_to_be_processed: dict of pcis with params to be processed.
    :return: None
    """
...
def process_devices(psid_map, pcis_to_be_processed, force_update_dict):
    ...
    # Set configuration according to dict of pcis with params to be processed.
    set_devices_config(pcis_to_be_processed)


def get_psid_map(binary_getter):
    """

    :param firmware_url: URL of a directory containing Nvidia NIC Firmware bins
    :return: dict mapping between PSID and an image_info dict
    """
    psid_map = {}
    if binary_getter:
        ...
        for fw_bin in fw_binaries:
            ...
            if psid_map.get(image_info['psid']):
                if psid_map[image_info['psid']]['fw_ver'] < \
                        image_info['fw_ver']:
...
def process_nvidia_nics(firmware_url, firmware_config):
    check_prereq()

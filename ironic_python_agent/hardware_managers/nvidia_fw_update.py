        ...
        if cfg.get("pcis_list"):
            ...
            for dev in nvidia_nics:
                if dev.dev_psid in psid_set:
                    temp_pci_list.append(dev)
        elif cfg.get("device_ids_list"):
            device_ids_set = set(cfg.get("device_ids_list"))
            for dev in nvidia_nics:
                if dev.dev_id in device_ids_set:
                    temp_pci_list.append(dev)
        else:
            for dev in nvidia_nics:
                temp_pci_list.append(dev)
...
def get_psid_map(bin_dir_url):
    ...
    :param bin_dir_url: URL of a directory containing Nvidia NIC Firmware bins
...
def process_nvidia_nics(bin_dir_url, yaml_config):
    check_prereq()

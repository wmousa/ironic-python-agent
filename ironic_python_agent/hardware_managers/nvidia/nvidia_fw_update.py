...
"""
Example of config.json configuration file:
[
  {
    "selectors": {
      "devices": [
...
class NvidiaNicFirmwareOps(object):
    ...
    def is_image_changed(self):
        ...
        :return: bool, True if image changed
        """
...
def filter_cfgs(nvidia_nics, cfgs):
    ...
    for cfg in cfgs:
        ...
        if selectors.get("devices"):
            filtered_devs = filter_by_selector(filtered_devs,
                                               cfg["selectors"]["devices"],
                                               "dev_id")
        if selectors.get("psids"):
            filtered_devs = filter_by_selector(filtered_devs,
                                               cfg["selectors"]["psids"],
                                               "dev_psid")
        if selectors.get("pcis"):
            filtered_devs = filter_by_selector(filtered_devs,
                                               cfg["selectors"]["pcis"],
                                               "dev_pci")
        ...
        for dev in filtered_devs:
            ...
            if is_first_device and cfg.get("function0Config"):
                params.update(cfg.get("function0Config"))
            elif not is_first_device and cfg.get("function1Config"):
                params.update(cfg.get("function1Config"))
...
def fw_update_if_needed(psid_map, force_update_dict):
    ...
    for dev, force_update in force_update_dict.items():
        try:
            ...
            # Note: device Reset is required if a different firmware version
            # was burnt
            if dev_ops.is_image_changed():
...
def get_psid_map(binary_getter):
    """Get psid from firmware_url directory

    :param binary_getter: an instance of NvidiaFirmwareBinariesFetcher
    class which fetches Nvidia NIC Firmware bins from firmware_url
